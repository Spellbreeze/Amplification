...
//Channel parameters
#define CHANNEL_SCALING               4
#define TX_INTERVAL_DEF               (0x00030000>>CHANNEL_SCALING)
#define SYNC_TIME_MASK_DEF            (0x0003FFFF>>CHANNEL_SCALING)
...
ADDR_PTR maccess(ADDR_PTR addr);     //Load Address
CYCLES   maccess_t(ADDR_PTR addr);   //Load And Time the Access
void     mwrite_v(ADDR_PTR addr, uint64_t value); //Write a Value to Address
uint64_t rdtscp(void);               //Get Timestamp
uint64_t rdtsc_cripple(uint64_t);    //Get Timestamp with inputted granularity (ShowTime)
uint64_t rdtsc_begin(void);         //Get Timestamp start (leaky.page style)
uint64_t rdtsc_end(void);           //Get Timestamp stop (leaky.page style)
void     lfence();
void     mfence();
void     lmfence();
